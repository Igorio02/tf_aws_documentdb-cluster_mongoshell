...
```
 4) During `terraform plan` or `terraform apply` execution you have to set value for `var.master_passwd`. It's cluster endpoint password and we can specify it in the `variables.tf` if needed.
